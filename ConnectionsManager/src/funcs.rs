...
pub fn del_proxy_port(port: usize) -> Result<(), io::Error> {
    ...
    fs::remove_file(format!("/etc/systemd/system/proxy{}.service", port)).unwrap();
    Ok(())
}
...
pub fn add_stunnel_port(port: usize, ipv6: bool) -> std::result::Result<(), io::Error> {
    ...
    let commands = [
        format!("grep -qE '^(::|0\\.0\\.0\\.0:)?{port_str}$' /etc/stunnel/stunnel.conf || echo 'accept = {prefix}{port_str}' | sudo tee -a /etc/stunnel/stunnel.conf"),
