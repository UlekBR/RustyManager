...
fn main() {
    ...
    if args.len() <= 1 {
        loop {
            ...
            let memory = run_command_and_get_output("awk -F: '$3 >= 1000 { C++ } END { print C+0 }' /etc/passwd");
            println!("Memoria em uso: {}", memory);
            let cpu = run_command_and_get_output("awk -F: '$3 >= 1000 { C++ } END { print C+0 }' /etc/passwd");
